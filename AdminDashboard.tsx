...
import { Check, X, Shield, Users, DollarSign, MessageSquare, Box, BarChart, Settings, Plus, Play, Pause, Trash2, Edit, Save, Image as ImageIcon, CreditCard, Lock, Key, Gift, Wrench, BookOpen, Layers, EyeOff, RefreshCw, ChevronLeft, Search, UserCheck, UserX, Clock, Palette, FileText } from 'lucide-react';
...
type AdminTab = 'dashboard' | 'products' | 'bonuses' | 'affiliates' | 'social_proof' | 'settings' | 'settings_checkout' | 'settings_support' | 'settings_theme' | 'settings_legal' | 'users' | 'withdrawals' | 'plans' | 'library';
                ...
                <div key={cat.id} className="bg-black rounded-lg border border-gray-800 overflow-hidden">
                  <div className="p-4 flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={async () => { await deleteCategory(cat.id); await refreshCategories(); toast.success('Categoria desativada!'); }} title="Desativar">🔴</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 border-t border-gray-800">
                    <button
                      onClick={() => {
                        setEditingSubcategory({
                          ...
                          name: '',
                          description: '',
                          order: subcategories.filter(s => s.categoryId === cat.id).length + 1,
                          dripDays: 0,
                          active: true,
                        });
                    ...
                    >
                      <Plus size={11} /> Add Sub-Categoria
                    </button>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <button 
                  onClick={() => {
                    ...
                    if (!isNew) {
                      ...
                      // Abre tela de nova subcategoria vinculada a esta categoria
                      setEditingSubcategory({
                        id: Math.random().toString(),
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-gray-800 pb-2 gap-2 flex-wrap">
                  <h4 className="text-[#D4AF37] font-bold">Subcategorias (Sagas)</h4>
                  <div className="flex gap-2 flex-wrap">
                    {showInactiveSubcategories ? (
                      <button onClick={() => setShowInactiveSubcategories(false)} className="text-[10px] bg-gray-700 text-white px-2 py-1 rounded hover:bg-gray-600">Ver ativas</button>
                    ) : (
                      <button onClick={() => setShowInactiveSubcategories(true)} className="text-[10px] bg-gray-800 text-gray-400 px-2 py-1 rounded hover:bg-gray-700">Ver desativadas</button>
                    )}
                    {!showInactiveSubcategories && (
                      <button
                        onClick={() => {
                          setEditingSubcategory({
                            ...
                            name: '',
                            description: '',
                            order: subcategories.filter(s => s.categoryId === editingCategory.id).length + 1,
                            dripDays: 0,
                            active: true,
                          });
                          setIsEditingSubcategory(true);
                ...
                {!showInactiveSubcategories ? (
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                    {subcategories.filter(s => s.categoryId === editingCategory.id).length === 0 ? (
                      <p className="text-gray-500 text-xs text-center py-4">Nenhuma subcategoria nesta categoria.</p>
                    ) : (
                      subcategories.filter(s => s.categoryId === editingCategory.id)
                        .sort((a, b) => a.order - b.order)
                        .map(sub => (
                          <div key={sub.id} className="bg-black p-3 rounded-lg border border-gray-800 flex items-center gap-3">
                            <div className="flex-1 min-w-0">
                              <h5 className="text-white font-bold text-xs truncate">{sub.name}</h5>
                              <p className="text-gray-500 text-[10px]">Ordem: {sub.order}</p>
                            </div>
                            <div className="flex gap-2">
                              <button onClick={() => { setEditingSubcategory(sub); setIsEditingSubcategory(true); }} className="text-gray-400 hover:text-white"><Edit size={14} /></button>
                              <button onClick={async () => { await deleteSubcategory(sub.id); await refreshCategories(); toast.success('Subcategoria desativada!'); }} title="Desativar">🔴</button>
                            </div>
                          </div>
                        ))
                    )}
        ...
        return (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
              {/* Header — igual ao header de Categorias */}
              <div className="p-4 border-b border-gray-800 flex flex-wrap justify-between items-center gap-2">
                <h2 className="text-lg font-bold text-white flex items-center gap-2 shrink-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => setListingSubsForCategory(null)} className="text-gray-400 hover:text-white ml-1"><X size={18} /></button>
                </div>
              </div>
              {/* Grid */}
                ...
                {!showInactiveSubcategories ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {subcategories.filter(s => s.categoryId === listingSubsForCategory).length === 0 ? (
                      <p className="text-gray-500 text-sm text-center py-6 col-span-2">Nenhuma subcategoria nesta categoria.</p>
                    ) : (
                      subcategories.filter(s => s.categoryId === listingSubsForCategory)
                        .sort((a, b) => a.order - b.order)
                        .map(sub => (
                          <div key={sub.id} className="bg-black rounded-lg border border-gray-800 overflow-hidden">
                            <div className="p-4 flex justify-between items-start">
                              <div>
                                <h3 className="text-white font-bold">{sub.name}</h3>
                                <p className="text-gray-500 text-xs">{sub.description}</p>
                                {sub.dripDays ? <span className="bg-blue-500/20 text-blue-500 text-[10px] px-2 py-0.5 rounded mt-2 inline-block">Drip: {sub.dripDays}d</span> : null}
                              </div>
                              <div className="flex gap-2">
                                <button onClick={() => { setEditingSubcategory(sub); setListingSubsForCategory(null); setIsEditingSubcategory(true); }} className="text-gray-400 hover:text-white"><Edit size={16} /></button>
                                <button onClick={async () => { await deleteSubcategory(sub.id); await refreshCategories(); toast.success('Subcategoria desativada!'); }} title="Desativar">🔴</button>
                              </div>
                            </div>
                            <div className="grid grid-cols-2 border-t border-gray-800">
                              <button
                                onClick={() => {
                                  setEditingEbook({
                                    ...
                                    coverImage: '/img/capa.png',
                                    order: ebooks.filter(e => e.subcategoryId === sub.id).length + 1,
                                    active: true,
                                    content: '',
                                  });
                                  setListingSubsForCategory(null);
                                  setIsEditingEbook(true);
                                }}
                                className="flex items-center justify-center gap-1 py-2 text-[11px] text-white font-semibold border-r border-gray-800 hover:bg-gray-800 transition-colors"
                              >
                                <Plus size={11} /> Add E-book
                              </button>
                              <button
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    if (isNew) {
                      await addSubcategory(editingSubcategory as any);
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <FileUpload
                  ...
                  accept=".pdf"
                  label="Upload PDF"
                  categorySlug={categories.find(c => c.id === editingEbook.categoryId)?.name}
                  subcategorySlug={subcategories.find(s => s.id === editingEbook.subcategoryId)?.name}
                />
                <div className="mt-2">
                  <input 
                    type="text"
  ...
  const renderSettings = () => (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={() => toast.success('Configurações salvas!')} className="w-full bg-[#D4AF37] text-black font-bold py-2 rounded hover:bg-[#b5952f] mt-4">
  ...
  const renderSettingsCheckout = () => (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={() => toast.success('Integrações salvas!')} className="w-full bg-[#D4AF37] text-black font-bold py-2 rounded hover:bg-[#b5952f] mt-4">
          Salvar Integrações
        </button>
      </div>
    </div>
  ...
  const renderSettingsSupport = () => (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={() => toast.success('Canais salvos!')} className="w-full bg-[#D4AF37] text-black font-bold py-2 rounded hover:bg-[#b5952f] mt-4">
          Salvar Canais
        </button>
      </div>
    </div>
  ...
  const renderSettingsTheme = () => (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={() => toast.success('Tema salvo!')} className="w-full bg-[#D4AF37] text-black font-bold py-2 rounded hover:bg-[#b5952f] mt-4">
          Salvar Tema
        </button>
      </div>
    </div>
  ...
  const renderSettingsLegal = () => (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={() => toast.success('Textos salvos!')} className="w-full bg-[#D4AF37] text-black font-bold py-2 rounded hover:bg-[#b5952f] mt-4">
          Salvar Textos
        </button>
      </div>
    </div>
                ...
                <div className="bg-gray-900 rounded-xl p-8 border border-gray-800 text-center">
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory" style={{scrollbarWidth: 'none'}}>
                    {products.map(product => (
                      <div key={product.id} className="bg-black rounded-lg border border-gray-800 overflow-hidden flex-shrink-0 snap-start" style={{width: '100%'}}>
