...
export const PlansManager: React.FC = () => {
  ...
  const handleAdd = () => {
    setIsAdding(true);
    setEditingId(null);
    setFormData({ name: '', price: 0, oldPrice: 0, period: 'único', features: [], isPopular: false, active: true, paymentLink: '' });
  };
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs text-gray-500 uppercase mb-1">Link de Pagamento (Cartão)</label>
                <input
                  type="url"
                  value={formData.paymentLink || ''}
                  onChange={e => setFormData({...formData, paymentLink: e.target.value})}
                  placeholder="https://checkout.stripe.com/... ou outro link"
                  className="w-full bg-black border border-gray-800 rounded-lg p-2 text-white text-sm"
                />
                <p className="text-[10px] text-gray-600 mt-1">Ao clicar em "Cartão" no checkout, o cliente será redirecionado para este link.</p>
              </div>
