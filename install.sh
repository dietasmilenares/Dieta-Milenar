...
# Garante que o diretório de log existe e pertence ao usuário da app
...
# Geração de serviço systemd — tenta caminhos comuns do pm2
...
if runuser -l "$APP_USER" -c "$PM2_BIN list" | grep -q "dieta-milenar.*online"; then
    ...
    log_error "FALHA CRITICA: Aplicacao Node travou (Crash Loop). Veja logs acima."
fi
...
# --- Gera config Nginx conforme modo (IP ou domínio+SSL) ---
if [[ "$USE_SSL" == true ]]; then

  # Primeiro: bloco temporário só na 80 para o Certbot validar o domínio
...
NGINX
    ...
    nginx -t >/dev/null
    systemctl reload nginx 2>/dev/null || systemctl restart nginx
    log_status "Nginx configurado com SSL."

  else
    log_warn "Certificado SSL não encontrado. Nginx configurado sem SSL."
...
NGINX
  ...
  # Modo IP — apenas porta 80, sem SSL
  cat > "/etc/nginx/sites-available/dieta-milenar" <<NGINX
server {
    listen 80;
    server_name ${DOMAIN};
...
# =============================================================================
#  MENU PÓS-INSTALAÇÃO
#  Para remover: apague daqui até o fim do arquivo
# =============================================================================
