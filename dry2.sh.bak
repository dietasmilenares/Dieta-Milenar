...
if [[ -f "$INSTALL_DIR/server.ts" && ! -f "$INSTALL_DIR/dist/server.js" ]]; then  
  log_status "Convertendo server.ts para JavaScript nativo de produção (esbuild com CJS)..."  
  # CORREÇÃO APLICADA AQUI: Mudando --format=esm para --format=cjs  
  runuser -l "$APP_USER" -c "cd $INSTALL_DIR && npx esbuild server.ts --bundle --platform=node --format=cjs --packages=external --outfile=dist/server.js >/dev/null 2>&1" || \  
  runuser -l "$APP_USER" -c "cd $INSTALL_DIR && npx tsc server.ts --outDir dist >/dev/null 2>&1" || true  
fi  
  
# Adicionando verificação explícita para dist/server.js  
if [[ ! -f "$INSTALL_DIR/dist/server.js" ]]; then  
  log_error "FALHA CRÍTICA: O arquivo dist/server.js não foi gerado. Verifique os logs de build."  
fi  
